...
echo -e "|=====================================================================|$W"     
echo ""
echo -e "$G Ubuntu Server 16.x$W"
echo ""
echo -e "$G== Init Config$W" 
echo ""
if [[ $EUID -ne 0 ]]; then
  echo -e "$R Error! Use user root$W" 
exit 0 
fi

ARQ=`uname -m`
if [ $ARQ != x86_64 ] ; then
   echo -e "$R Error! Not is 64 bits$W"
exit 0
fi

DHCP=`cat /etc/network/interfaces 2> /dev/null | grep "dhcp" | wc -l`
if [ $DHCP != 0 ]; then
   echo -e "$R Error! Use IP static in config /etc/network/interfaces$W"
exit 0
fi	

CHKETH=`cat /etc/network/interfaces 2> /dev/null | grep "eth0" | wc -l`
if [ $CHKETH == 0 ]; then
   echo -e "$R Error! No exist interface eth0$W"
exit 0
fi

CHKIFCONF=`ifconfig eth0 2> /dev/null | grep "eth0" | wc -l`
if [ $CHKIFCONF == 0 ]; then
   echo -e "$R Error! No exist value ifconfig eth0$W"
exit 0
fi
...
if [[ $DIST == "Debian" ]]; then
IPSERV=$(ifconfig eth0 | grep 'inet' | cut -d: -f2 | awk '{ print $2}' | xargs)		
fi
if [[ $DIST == "Ubuntu" ]]; then
IPSERV=$(ifconfig eth0 | grep 'inet:' | cut -d: -f2 | awk '{ print $1}')
fi
...
apt-get update -y
if [ $? -ne 0 ]; then
   echo -e "$R Error! Internet Connection$W"
exit 0
fi
apt-get upgrade -y
...
/etc/init.d/squid3 restart
if [ $? -ne 0 ]; then
   echo -e "$R Error! Squid3 restart$W"
fi
/etc/init.d/raptor start
if [ $? -ne 0 ]; then
   echo -e "$R Error! Raptor restart$W"
fi
if [ ! -f /var/run/raptor.pid ]; then
	ln -s /run/raptor.pid /var/run/raptor.pid
fi
sleep 1
echo "== Done Raptor2"
...
if [ $? -ne 0 ]; then
   echo "Error! Internet Connection, not downloaded webpanel"
   exit 0
fi
...
if [ $? -ne 0 ]; then
   echo -e "$R Error! Apache2 restart$W"
fi
clear
sleep 1
# == 
